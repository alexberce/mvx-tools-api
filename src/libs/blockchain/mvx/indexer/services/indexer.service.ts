...
export class IndexerService {
  ...
  constructor(private httpService: HttpService, private readonly config: ConfigService) {
    this.INDEX_URL = config.get('MVX_ES_INDEX_HOST');
  }

  queryPointInTime(pointInTimeId: string, options: unknown): Promise<QueryPitResponse<unknown>> {
    const url = `${this.INDEX_URL}/_search`;
    return lastValueFrom(this.httpService.get(url, {data: options})).then(response => response.data);
  }
  ...
  createPointInTime(index: string): Promise<CreatePitResponse> {
    ...
    return lastValueFrom(this.httpService.post(url)).then(response => response.data);
  }
}
